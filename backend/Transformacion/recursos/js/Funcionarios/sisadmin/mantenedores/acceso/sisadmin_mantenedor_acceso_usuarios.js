// Mantenedor de Usuarios de Acceso
let allData = [];
...
window.loadData = async function () {
    ...
    try {
        const response = await fetch(`${window.API_BASE_URL}/sisadmin/mantenedores/acceso/usuarios.php`, {
            method: 'POST',
            credentials: 'include',
...
function renderTable(dataItems) {
    ...
    dataItems.forEach(item => {
        ...
        const creationDate = item.usr_creacion ? new Date(item.usr_creacion).toLocaleDateString() : 'N/A';
        row.innerHTML = `
            <td class="fw-bold">${item.usr_rut}</td>
            <td>${item.usr_nombre} ${item.usr_apellido}</td>
            <td><small>${item.usr_email || '-'}</small></td>
            <td><small>${creationDate}</small></td>
            <td class="text-end">
                <button class="btn btn-sm btn-outline-secondary me-1" onclick="editItem(${item.usr_id})">
                    <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg>
                </button>
                <button class="btn btn-sm btn-outline-danger" onclick="deleteItem(${item.usr_id})">
...
function filterData() {
    ...
    const filtered = allData.filter(item => {
        const textMatch = !filterVal ||
            (item.usr_rut && item.usr_rut.toLowerCase().includes(filterVal)) ||
            (item.usr_nombre && item.usr_nombre.toLowerCase().includes(filterVal)) ||
            (item.usr_apellido && item.usr_apellido.toLowerCase().includes(filterVal));
        return textMatch;
    });
...
window.openModal = function (mode, data = null) {
    ...
    if (mode === 'create') {
        ...
        if (idInput) idInput.value = data.usr_id;
        document.getElementById('entry-rut').value = data.usr_rut;
        document.getElementById('entry-nombre').value = data.usr_nombre;
        document.getElementById('entry-apellido').value = data.usr_apellido;
        document.getElementById('entry-email').value = data.usr_email || '';
    }
...
window.editItem = function (id) {
    const item = allData.find(o => o.usr_id == id);
    ...
    if (result.isConfirmed) {
        try {
            const response = await fetch(`${window.API_BASE_URL}/sisadmin/mantenedores/acceso/usuarios.php`, {
                ...
                credentials: 'include',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ ACCION: 'BORRAR', usr_id: id })
            });
...
async function saveData() {
    ...
    const apellido = document.getElementById('entry-apellido').value;
    ...
    if (currentMode === 'edit') payload.usr_id = parseInt(id);
