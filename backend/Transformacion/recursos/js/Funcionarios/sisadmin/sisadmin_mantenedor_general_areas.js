...
function renderTable(dataItems) {
    ...
    dataItems.forEach(item => {
        ...
        row.innerHTML = `
            <td class="fw-bold text-muted">${item.tga_id}</td>
            <td><code>${item.tga_codigo_area}</code></td>
            <td>${item.tga_nombre}</td>
            <td class="text-end">
                <button class="btn btn-sm btn-outline-secondary me-1" onclick="editItem(${item.tga_id})">
                    <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg>
                </button>
                <button class="btn btn-sm btn-outline-danger" onclick="deleteItem(${item.tga_id})">
...
function filterData() {
    ...
    const filtered = allData.filter(item => {
        const textMatch = !filterVal ||
            (item.tga_codigo_area && item.tga_codigo_area.toLowerCase().includes(filterVal)) ||
            (item.tga_nombre && item.tga_nombre.toLowerCase().includes(filterVal));
        return textMatch;
    });
...
window.openModal = function (mode, data = null) {
    ...
    if (mode === 'create') {
        if (modalTitle) modalTitle.textContent = 'Nueva Área';
    } else if (mode === 'edit' && data) {
        if (modalTitle) modalTitle.textContent = 'Editar Área';
        if (idInput) idInput.value = data.tga_id;
        document.getElementById('entry-codigo').value = data.tga_codigo_area;
        document.getElementById('entry-nombre').value = data.tga_nombre;
    }
...
window.editItem = function (id) {
    const item = allData.find(o => o.tga_id == id);
    ...
    if (result.isConfirmed) {
        try {
            const response = await fetch(`${window.API_BASE_URL}/areas_general.php`, {
                ...
                credentials: 'include',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ ACCION: 'BORRAR', tga_id: id })
            });

            const data = await response.json();
            if (data.status === 'success') {
                Swal.fire('Éxito', 'Área eliminada correctamente.', 'success');
                loadData();
            } else {
