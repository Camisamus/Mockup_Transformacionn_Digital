...
document.addEventListener('DOMContentLoaded', async function () {
    ...
    const API_BASE_URL = window.API_BASE_URL || (window.location.origin + '/transformacion/apivec');
    ...
    try {
        // Fetch Inbox
        const fetchUrl = `${API_BASE_URL}/general/bandeja.php`;
        ...
        if (response.status === 401) {
            window.location.href = '../index.php'; // Redirigir al login si no está autorizado
            return;
        }

        // Cargar Funcionarios para búsqueda (si los vecinos pueden asignar tareas a funcionarios)
        const respFunc = await fetch(`${API_BASE_URL}/general/funcionarios.php`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        const fetchUrl2 = `${API_BASE_URL}/general/tareas.php`;
...
function abrirModalCrearTarea() {
    ...
    // Use neighbor session data if available, or placeholder
    const neighborData = JSON.parse(localStorage.getItem('vecino_data')) || { vecino: { nombre: 'Vecino', id: 0 } };
    document.getElementById('responsable').value = neighborData.vecino.nombre;
    document.getElementById('usr_id').value = neighborData.vecino.id;
...
function guardarTarea() {
    ...
    const data = { ACCION: 'CREAR' };
    formData.forEach((value, key) => data[key] = value);

    const API_BASE_URL = window.API_BASE_URL || (window.location.origin + '/transformacion/apivec');
    const fetchUrl = `${API_BASE_URL}/general/tareas.php`;
...
window.abrirModalBuscarFuncionario = abrirModalBuscarFuncionario;
