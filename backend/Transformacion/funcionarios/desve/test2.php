    ...
    <form id="form_modificar_desve">
        ...
        <div class="bg-white border border-slate-100 rounded-3xl p-6 lg:p-10 flex flex-col lg:flex-row justify-between items-start lg:items-center shadow-sm gap-6 sticky top-4 z-20">
            <div class="space-y-1 w-full text-left">
                <h1 class="text-2xl lg:text-3xl font-extrabold text-slate-800 tracking-tight" id="header_public_id">Modificar Solicitud</h1>
                <p class="text-slate-400 text-sm lg:text-[15px] font-medium" id="header_expediente">Actualice los campos requeridos de la solicitud.</p>
            </div>

            <div class="flex items-center gap-4 w-full lg:w-auto justify-end">
                <button type="button" id="btn_cancelar_toolbar" class="text-slate-400 font-bold hover:text-slate-600 transition-all text-[13px] uppercase tracking-wider">Cancelar</button>
                <button type="submit"
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 pt-4">
            ...
            <div class="lg:col-span-8 space-y-6">
                    ...
                    <div class="p-6 lg:p-10 space-y-8">
                        <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                            ...
                            <div class="space-y-2">
                                ...
                                <input type="text" id="Codigo_DESVE"
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="space-y-2">
                                ...
                                <div class="flex shadow-sm h-[48px]">
                                    ...
                                    <button type="button" id="btn_nuevo_origen" class="bg-slate-800 text-white px-4 rounded-r-xl flex items-center"><span class="material-symbols-outlined">add</span></button>
                                </div>
                            </div>
                        </div>
                ...
                <div class="bg-white gob-card rounded-2xl overflow-hidden">
                    ...
                    <div id="geolocalizacion_area" class="p-6 lg:p-10 space-y-6 hidden">
                        ...
                        <div id="map_desve" style="height: 400px;" class="w-full border border-slate-100 rounded-2xl shadow-inner"></div>
                    </div>
                </div>
                ...
                <div class="bg-white gob-card rounded-2xl overflow-hidden">
                    <div class="p-5 border-b border-slate-50 flex items-center justify-between bg-white">
                        ...
                        <button type="button" onclick="abrirModalBuscarFuncionario()" class="flex items-center gap-2 bg-slate-800 hover:bg-black text-white font-bold py-1.5 px-4 rounded-lg text-[10px] uppercase tracking-wider transition-all">
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-left text-[14px]" id="tabla_destinos">
                            <thead class="bg-slate-50 text-slate-500 font-bold uppercase text-[10px] tracking-widest border-b border-slate-100">
                                <tr><th class="px-6 py-3">Funcionario</th><th class="px-6 py-3">Email</th><th class="px-6 py-3 text-right">Acción</th></tr>
                            </thead>
                            <tbody id="tbody_destinos" class="divide-y divide-slate-50 text-slate-600 italic">
                                <tr id="placeholder_destinos"><td colspan="3" class="px-6 py-8 text-center text-slate-400">Cargando destinatarios...</td></tr>
                            </tbody>
                        </table>
                    </div>
                </div>
                ...
                <div class="bg-white gob-card rounded-2xl overflow-hidden">
                    <div class="p-5 border-b border-slate-50 bg-slate-50/50">
                        <div class="flex items-center gap-2">
                            ...
                            <h3 class="font-bold text-slate-700 uppercase text-sm tracking-wide">Bitácora de Respuestas</h3>
                        </div>
                    </div>
                    <div class="overflow-x-auto">
                        <table class="w-full text-left" id="tabla_respuestas">
                            ...
                            <tbody id="tbody_respuestas" class="divide-y divide-slate-50 text-[13px] text-slate-600"></tbody>
                        </table>
                    </div>
                </div>
            </div>
            ...
            <div class="lg:col-span-4 space-y-6">
                ...
                <div class="bg-[#D3D3D3] border border-slate-300 rounded-3xl p-8 shadow-sm space-y-6">
                    <h2 class="text-xs font-bold uppercase tracking-[0.2em] text-slate-500 flex items-center gap-2"><span class="material-symbols-outlined text-sm">settings_suggest</span> Estado y Tiempos</h2>
                    
                    <div class="space-y-3">
                        <label class="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Estado de Entrega</label>
                        <div class="flex p-1 bg-white/50 rounded-xl border border-slate-200 shadow-inner">
                            <input type="radio" name="EstadoEntrega" id="estadoPendiente" value="0" class="hidden peer/p">
                            <label for="estadoPendiente" class="flex-1 text-center py-2 text-[11px] font-bold rounded-lg cursor-pointer peer-checked/p:bg-gob-warning peer-checked/p:text-white text-slate-500 transition-all uppercase">Pendiente</label>

                            <input type="radio" name="EstadoEntrega" id="estadoRespondido" value="1" class="hidden peer/r">
                            <label for="estadoRespondido" class="flex-1 text-center py-2 text-[11px] font-bold rounded-lg cursor-pointer peer-checked/r:bg-gob-success peer-checked/r:text-white text-slate-500 transition-all uppercase">Respondido</label>
                        </div>
                    </div>

                    <div class="grid grid-cols-2 gap-4 border-t border-slate-300 pt-6">
                        <div class="space-y-1">
                            <label class="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Días Transcurridos</label>
                            <input type="text" id="info_dias_ingreso" readonly class="w-full bg-white/40 border-none rounded-xl text-lg font-black text-slate-700 text-center focus:ring-0" />
                        </div>
                        <div class="space-y-1">
                            <label class="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Restantes</label>
                            <input type="text" id="info_dias_vencimiento" readonly class="w-full bg-white/40 border-none rounded-xl text-lg font-black text-slate-700 text-center focus:ring-0" />
                        </div>
                    </div>
                </div>

                <div class="bg-soft-cyan border border-cyan-border rounded-2xl overflow-hidden shadow-sm">
                    <div class="p-5 border-b border-cyan-border flex items-center gap-2"><span class="material-symbols-outlined text-primary-blue text-[20px]">attach_file</span><h3 class="font-bold text-slate-700 uppercase text-xs tracking-widest">Documentos</h3></div>
                    <div class="p-6 space-y-4">
                        <div id="lista_documentos_guardados" class="space-y-2 mb-4"></div>
                        <div class="drop-zone" id="drop_zone">
                            <input type="file" id="inputArchivosSolicitud" hidden multiple><span class="material-symbols-outlined text-slate-300 text-3xl mb-1">cloud_upload</span><p class="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Subir nuevos archivos</p>
                        </div>
        ...
        <input type="hidden" id="OrigenSolicitud">
    ...
    <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content border-0 shadow-2xl rounded-3xl overflow-hidden">
            <div class="modal-header bg-slate-50 border-b p-6">
                <h5 class="modal-title font-extrabold text-slate-800 text-sm uppercase tracking-widest">Buscar Funcionario</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <div class="p-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                    <input type="text" class="rounded-xl border-slate-200 text-sm" id="buscar_fnc_input" placeholder="Nombre o apellido...">
                ...
                <div class="table-responsive max-h-[400px] rounded-xl border border-slate-100">
                    <table class="table table-hover align-middle mb-0">
                        <thead class="bg-slate-50 text-[10px] font-bold text-slate-400 uppercase border-b">
                            <tr><th>Email</th><th>Nombre</th><th class="text-end">Acción</th></tr>
                        </thead>
...
<div class="modal fade" id="modalReingreso" tabindex="-1">
    <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content border-0 shadow-2xl rounded-3xl overflow-hidden">
            <div class="modal-header bg-slate-50 border-b p-6">
                <h5 class="modal-title font-extrabold text-slate-800 text-sm uppercase tracking-widest">Buscar Solicitud Previa</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <div class="p-6">
                <input type="text" class="w-full rounded-xl border-slate-200 text-sm mb-4" id="filtroReingreso" placeholder="Código DESVE o nombre...">
                <div class="table-responsive max-h-[400px] rounded-xl border border-slate-100">
                    <table class="table table-hover align-middle mb-0">
                        <thead class="bg-slate-50 text-[10px] font-bold text-slate-400 uppercase border-b">
                            <tr><th>Código</th><th>Expediente</th><th class="text-end">Acción</th></tr>
                        </thead>
